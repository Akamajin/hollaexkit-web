...
class UserFinancials extends Component {
	...
	getBalanceColumn = () => {
		return [
			...
			},
			{
				...
				render: (id, rowData) => (
					<div>
						...
						{rowData.meta ? (
							<span
								onClick={() => this.showBankData(rowData)}
			...
			},
			{
				...
				key: 'end_date',
				render: (endDate) =>
					endDate ? <Moment format="YYYY/MM/DD">{endDate}</Moment> : '',
			},
			{
				title: '',
				dataIndex: 'id',
				...
				render: (id, rowData) => (
					<div>
						<Button
							type="primary"
							onClick={() => this.requestDelete(id)}
							className="mr-1"
							style={{ backgroundColor: '#ff0000' }}
						>
							Delete
						</Button>
						{rowData.action === 'Withdraw (Pending)' ||
						rowData.action === 'Withdraw Investment (Pending)' ? (
							<Button
								type="primary"
								onClick={() => this.acceptWithdraw(id, rowData.action)}
	...
	onChangeParams = (key) => (value) => {
		...
		if (value) {
			if (key === 'created_at' || key === 'end_date') {
				...
				if (
					key === 'action' &&
					value !== 'Capital Investment (Fixed)' &&
					value !== 'Capital Investment (Decreasing)'
				) {
	...
	findLastGroup() {
		...
		return buttonsForGroups.length > 0
			? Math.max(...buttonsForGroups.map((bfg) => bfg.groupId))
			: -1;
	}
	...
	addRow = () => {
		...
		if (
			formData.action === 'Capital Investment (Fixed)' ||
			formData.action === 'Capital Investment (Decreasing)'
		)
			activeGroup = this.findLastGroup() + 1;
		if (formData.deduction_rate === '')
			formData.deduction_rate = formData.interest_rate;
	...
	requestDelete = (id) => {
		...
		if (
			(targetRow.action === 'Capital Investment (Fixed)' ||
				targetRow.action === 'Capital Investment (Decreasing)') &&
			currentGroup.length > 1
		) {
			Modal.info({
				content: (
					<div>
						This is a core record and cannot be removed. please remove normal
						records first.
					</div>
				),
			});
			...
			Modal.confirm({
				...
				onOk() {
					deleteFinanacialById(id)
						.then((res) => {
							if (currentGroup.length === 1 && buttonsForGroups.length > 0) {
								$this.setState(
									{ activeGroup: buttonsForGroups[0].groupId },
									() => {
										$this.fetchData();
									}
								);
			...
			content: (
				<div>
					<h4 className="mb-4" style={{ color: '#ffffff' }}>
						Bank and Wallet Info
					</h4>
					<div>
						<b>IBAN:</b> {parsedMeta.ei_iban}
					</div>
					<div>
						<b>Bank Name:</b> {parsedMeta.ei_bank_name}
					</div>
					<div>
						<b>Holder Full Name:</b> {parsedMeta.ei_holder_name}
					</div>
					<div>
						<b>Account Number:</b> {parsedMeta.ei_acc_no}
					</div>
					<div>
						<b>USDT TRC Address:</b> {parsedMeta.ei_trc_address}
					</div>
				</div>
			),
		});
	...
	acceptWithdraw = (id, action) => {
		updateBalanceRow({ id, action: action.replace(' (Pending)', '') })
			.then((res) => {
				this.fetchData();
			})
			.catch((err) => {
	...
	fetchData = () => {
		...
		getUserFinancials(this.props.userData.id).then((res) => {
			this.setState(
				{ loading: false, financialData: res.data },
	...
	updateTableData = () => {
		...
		for (const groupId in groups) {
			...
			buttonsForGroups.push({
				...
				title: `${groupId} - ${baseAction.action
					.split(' ')[2]
					.replace('(', '')
					.replace(')', '')} ($${baseAction.amount}@${
					baseAction.interest_rate
				}%${
					baseAction.deduction_rate &&
					baseAction.interest_rate !== baseAction.deduction_rate
						? ',' + baseAction.deduction_rate + '%'
						: ''
				})`,
			});
		...
		return (
			<div className="f-1 admin-user-container admin-financials-wrapper">
				...
				<div className="investment-buttons">
					...
					<div>
						<button
							className="ant-btn ant-btn-primary mr-3"
							onClick={() => this.requestCalc()}
						>
							Calcualte interests
						</button>
						<button
